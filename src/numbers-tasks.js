...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const numeratorOne = Math.sqrt(x1 ** 2 + y1 ** 2);
  const numeratorTwo = Math.sqrt(x2 ** 2 + y2 ** 2);
  const denominator = numeratorOne * numeratorTwo;
...
function isPowerOfTwo(num) {
  ...
  const parseOne = Math.ceil(Math.log(num) / Math.log(2));
  const parseTwo = Math.floor(Math.log(num) / Math.log(2));
  return parseInt(parseOne, 10) === parseInt(parseTwo, 10);
}
